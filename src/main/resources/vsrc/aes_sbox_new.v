...
module MUX31I ( A, B, C , s, Q );
input A;
input B;
input C;
input [1:0]s;
output Q;
assign Q = ~ ( (s == 1)? A : (s == 0)? B : C ); /* mock-up for FPGA implementation */
endmodule

/* select and invert (NOT) byte, using MUX21I */
module SELECT_NOT_8 ( A, B, C, s, Q );
input [7:0] A;
input [7:0] B;
input [7:0] C;
input [1:0]s;
output [7:0] Q;
MUX31I m7(A[7],B[7],C[7],s,Q[7]);
MUX31I m6(A[6],B[6],C[6],s,Q[6]);
MUX31I m5(A[5],B[5],C[5],s,Q[5]);
MUX31I m4(A[4],B[4],C[4],s,Q[4]);
MUX31I m3(A[3],B[3],C[3],s,Q[3]);
MUX31I m2(A[2],B[2],C[2],s,Q[2]);
MUX31I m1(A[1],B[1],C[1],s,Q[1]);
MUX31I m0(A[0],B[0],C[0],s,Q[0]);
endmodule
/* find either Sbox or its inverse in GF(2^8), by Canright Algorithm */
module bSbox ( A, encrypt, Q );
input [7:0] A;
input [1:0] encrypt; /* 1 for Sbox, 0 for inverse Sbox */
output [7:0] Q;
wire [7:0] B, C, D, X, Y, Z , SM_B, SM_D;
...
assign B[1] = R4 ;
assign B[0] = A[2] ~^ R9 ;
// assign SM_B[7] = ~(A[1] ^ A[2] ^ A[4] ^ A[6] ^ 1);
// assign SM_B[6] = ~(A[2] ^ A[4] ^ A[5] ^ 1);
// assign SM_B[5] = ~(A[0] ^ A[2] ^ A[5] ^ A[7] ^ 1);
// assign SM_B[4] = ~(A[0] ^ A[1] ^ A[2] ^ A[3] ^ A[4] ^ A[7]  ^ 1); 
// assign SM_B[3] = ~(A[1] ^ A[2] ^ A[3] ^ A[4] ^ A[5] ^ A[6]);
// assign SM_B[2] = ~(A[0] ^ A[1] ^ A[2] ^ A[5] ^ A[7] ^ 1);
// assign SM_B[1] = ~(A[0] ^ A[3] ^ 1);
// assign SM_B[0] = ~(A[0] ^ A[1] ^ A[2] ^ A[4] ^ A[5] ^ A[6]^ A[7] ^ 1);
assign SM_B[7] = A[1] ^ A[2] ^ A[4] ^ A[6];
assign SM_B[6] = A[2] ^ A[4] ^ A[5] ;
assign SM_B[5] = A[0] ^ A[2] ^ A[5] ^ A[7];
assign SM_B[4] = A[0] ^ A[1] ^ A[2] ^ A[3] ^ A[4] ^ A[7] ; 
assign SM_B[3] = ~(A[1] ^ A[2] ^ A[3] ^ A[4] ^ A[5] ^ A[6]);
assign SM_B[2] = A[0] ^ A[1] ^ A[2] ^ A[5] ^ A[7];
assign SM_B[1] = A[0] ^ A[3] ;
assign SM_B[0] = A[0] ^ A[1] ^ A[2] ^ A[4] ^ A[5] ^ A[6]^ A[7];
assign Y[7] = R2 ;
assign Y[6] = A[4] ^ R8 ;
assign Y[5] = A[6] ^ A[4] ;
assign Y[4] = R9 ;
...
assign Y[1] = A[4] ^ R6 ;
assign Y[0] = A[1] ^ R5 ;
SELECT_NOT_8 sel_in( B, Y, SM_B , encrypt, Z );
GF_INV_8 inv( Z, C );
...
assign X[1] = T6 ;
assign X[0] = ~ C[2] ;
// assign SM_D[7] =  ~(C[7] ^ C[1] ^ C[0] ^ 1);
// assign SM_D[6] =  ~(C[7] ^ C[5] ^ C[2] ^ 1);
// assign SM_D[5] =  ~(C[7] ^ C[5] ^ C[3] ^ C[2]);
// assign SM_D[4] =  ~(C[7] ^ C[6] ^ C[3] ^ C[1] ^ 1);
// assign SM_D[3] =  ~(C[7] ^ C[6] ^ C[4] ^ C[3] ^ C[1]);
// assign SM_D[2] =  ~(C[7] ^ C[6] ^ C[3] ^ C[2] ^ C[1] ^ C[0]);
// assign SM_D[1] =  ~(C[6] ^ C[4] ^ C[1] ^ 1);
// assign SM_D[0] =  ~(C[7] ^ C[6] ^ C[5] ^ C[4] ^ C[1] ^ 1);
assign SM_D[7] =  C[7] ^ C[1] ^ C[0] ;
assign SM_D[6] =  C[7] ^ C[5] ^ C[2] ;
assign SM_D[5] =  ~(C[7] ^ C[5] ^ C[3] ^ C[2]);
assign SM_D[4] =  C[7] ^ C[6] ^ C[3] ^ C[1] ;
assign SM_D[3] =  ~(C[7] ^ C[6] ^ C[4] ^ C[3] ^ C[1]);
assign SM_D[2] =  ~(C[7] ^ C[6] ^ C[3] ^ C[2] ^ C[1] ^ C[0]);
assign SM_D[1] =  C[6] ^ C[4] ^ C[1];
assign SM_D[0] =  C[7] ^ C[6] ^ C[5] ^ C[4] ^ C[1];
SELECT_NOT_8 sel_out( D, X, SM_D , encrypt, Q );
endmodule
